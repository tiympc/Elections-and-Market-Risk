...
**We made a table for each individual beta. It is now a good idea to make a table for some other information and make our analysis more extensive. Therefore, we will look into political parties more closely.**
```{r}
...
**Let us go further into these five industries and introduct a different data set.**
```{r}
...
ggplot(data = landslideData, aes(x = percentDiff, y = values_technologyBeta)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
ggplot(data = landslideData, aes(x = percentDiff, y = values_healthCareBeta)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
ggplot(data = landslideData, aes(x = percentDiff, y = values_realEstateBeta)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
```{r}
90 before, 30 after and compare beta and mean return for each, then compare to above
```
